...
class L5SwaggerServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        $this->app->singleton(
            'command.l5-swagger.publish', function () {
            return new PublishCommand();
        }
        );
        $this->app->singleton(
            'command.l5-swagger.publish-config', function () {
            return new PublishConfigCommand();
        }
        );
        $this->app->singleton(
            'command.l5-swagger.publish-views', function () {
            return new PublishViewsCommand();
        }
        );
        $this->app->singleton(
            'command.l5-swagger.publish-assets', function () {
            return new PublishAssetsCommand();
        }
        );
        $this->app->singleton(
            'command.l5-swagger.generate', function () {
